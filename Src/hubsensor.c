...
void Hubsensor_Service (Hubsensor_t* HS_data){
	...
	if (i>8&&(UART2_RxBuff[i-8]+UART2_RxBuff[i-7]+UART2_RxBuff[i-6]+UART2_RxBuff[i-5]+UART2_RxBuff[i-4]+UART2_RxBuff[i-3])%256 == UART2_RxBuff[i-2]+1){
		HAL_GPIO_TogglePin(LED_GPIO_Port, LED_Pin);
		HS_data->HS_UARTFail = 0;
		HS_data->HS_Temperature = UART2_RxBuff[i-8]-30; //Byte 2: 0~210 represents -30℃~180℃, for example：30—0℃
		HS_data->HS_Pedalposition = UART2_RxBuff[i-6]&127;
		HS_data->HS_Pedals_turning = UART2_RxBuff[i-6]>>7;
		//filter torque value
		torque_recent =(((UART2_RxBuff[i-5]>>6)<<8)+UART2_RxBuff[i-7])-torque_offset;
		...
		HS_data->HS_Wheel_turning = (UART2_RxBuff[i-5]>>5)&1;
		if(((UART2_RxBuff[i-5]&31)<<8)+(UART2_RxBuff[i-4])<4501){ //safety reason, filter non plausible values
			HS_data->HS_Wheeltime = ((UART2_RxBuff[i-5]&31)<<8)+(UART2_RxBuff[i-4]);
