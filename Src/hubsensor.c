...
void Hubsensor_Service (Hubsensor_t* HS_data){
	...
	printf_("%d ,%d  %d\n ", i, (UART3_RxBuff[i-7]+UART3_RxBuff[i-6]+UART3_RxBuff[i-5]+UART3_RxBuff[i-4]+UART3_RxBuff[i-3])%256,UART3_RxBuff[i-2]+1);
	if ((UART3_RxBuff[i-7]+UART3_RxBuff[i-6]+UART3_RxBuff[i-5]+UART3_RxBuff[i-4]+UART3_RxBuff[i-3])%256 == UART3_RxBuff[i-2]+1){
		...
		if(torque_offset<(UART3_RxBuff[i-6]&127)){
			torque_cumulated+=((UART3_RxBuff[i-6]<<8)+UART3_RxBuff[i-6])-torque_offset;
			}
		else{
		...
		printf_("%d\r\n",(UART3_RxBuff[i-6]<<8)+UART3_RxBuff[i-6]);
	}
	else printf_("F\r\n");
}
